...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $password = $_POST['password']; // password mentah dulu buat cek admin

    // cek admin manual
    if ($username === 'kokwet' && $password === 'admin') {
        $_SESSION['username'] = 'kokwet';
        $_SESSION['nama'] = 'Admin';
        $_SESSION['admin'] = true;
    ...
    $passwordHash = md5($password); // sesuai yang dipakai waktu daftar
    $query = mysqli_query($koneksi, "SELECT * FROM users WHERE username='$username' AND password='$passwordHash'");

    if (mysqli_num_rows($query) > 0) {
        $data = mysqli_fetch_assoc($query);

        // simpan session data user biasa
    ...
    <div class="login-container">
        ...
        <form method="POST" autocomplete="off">
            ...
            <div class="register-link">
                <span style="color: #333;">Belum punya akun? </span>
                <a class="link-text" href="daftar.php">Daftar disini</a>
            </div>
